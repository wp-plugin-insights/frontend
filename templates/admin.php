...
<div class="container mt-4 pb-5" style="max-width:860px">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body p-0">
            ...
            <?php
            // Build a lookup: runner_slug → queue stats from RabbitMQ
            $queueByName = [];
            foreach ($queues as $q) {
                $queueByName[(string) ($q['name'] ?? '')] = $q;
            }

            // Merge runners with their result stats
            $runnerStatsBySlug = [];
            ...
            <div class="table-responsive border-bottom">
                <table class="table table-sm table-hover mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        $rQueue_  = $queueByName[$rQueue]    ?? null;
                        $rCount   = $rStats !== null ? (int) $rStats['result_count'] : 0;
                        $rLatest  = $rStats !== null ? substr((string) $rStats['latest_date'], 0, 16) : '—';
                        $rReady   = $rQueue_ !== null ? (int) $rQueue_['messages_ready'] : null;
                        $rConsumers = $rQueue_ !== null ? (int) $rQueue_['consumers'] : null;
                        ?>
                        <tr>
                            <td>
                ...
                <table class="table table-sm table-hover mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <?= esc($upName) ?>
                                <?php if ((string) ($up['plugin_slug'] ?? '') !== '') : ?>
                                <br><small class="text-body-secondary plugin-slug"><?= esc((string) $up['plugin_slug']) ?></small>
                                <?php endif; ?>
                            </td>
                            <td class="text-body-secondary small"><?= esc((string) ($up['plugin_version'] ?? '—')) ?></td>
                            <td>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            ...
            <p class="text-body-secondary small mb-0">No users found matching <em><?= esc($userSearchTerm) ?></em>.</p>
            <?php endif; ?>
